(async () => {
    try {
        ...
        console.log("IDs:", ids);
        if (!ids.walletId || !ids.issuerId)
            throw new Error("Missing IDs for Minting");
        const defs = await (await fetch('http://localhost:3000/api/v1/vcredits/definitions')).json();
        const skyMileDef = defs.find((d) => d.id === 'SKY_MILES_V2' || d.issuerId === ids.issuerId);
        if (!skyMileDef)
            throw new Error("No Credit Definition found for Mint");
...
//# sourceMappingURL=test_mint_v2.js.map